...
public class SelectorGenerator {
	...
	public static void generate(VirtualFile newFile, List<SelectorDetector.Result> detectorResults, VirtualFile folder) {
		Log.d("generating XML:");
		String path = "@drawable/";
		if(folder.getName().contains("mipmap")){
			path = "@mipmap/";
		}
		...
		for (SelectorDetector.Result result : detectorResults) {
			...
			Attribute attribute = new Attribute("drawable", path + result.drawableName);
